...
if (typeof gandenganLookupType !== 'undefined' && typeof gandenganLookupData !== 'undefined') {
    typeData = gandenganLookupType;
    filtersData = gandenganLookupData;

    if (parsePostData.penerimaanstok_id != '' && filtersData.length > 0 && parsePostData.penerimaanstok_id == filtersData[0].penerimaanStokPgForLookup && parsePostData.gandengandarike != '') {
        
        if (parsePostData.gandengandarike == 'dari' && parsePostData.gandenganke_id != '') {
            result = filtersData.filter((word) => word.id != parsePostData.gandenganke_id);            
        } else if (parsePostData.gandengandarike == 'ke' && parsePostData.gandengandari_id != '') {
            result = filtersData.filter((word) => word.id != parsePostData.gandengandari_id);
        } else {
            result = filtersData
        }
    } else {        
...
urlRequestGrid = parsePostData.url || `${apiUrl}`;
urlRequestGrid +='gandengan';
