...
if (typeof mingguanLookupType !== 'undefined' && typeof mingguanLookupData !== 'undefined') {
    typeData = mingguanLookupType;
    filtersData = mingguanLookupData;

    if (parsePostData.bulan && parsePostData.bulan != undefined) {
        const tgl = `01-${parsePostData.bulan}`
        const splitTgl = tgl.split('-')
        const tanggal_dari = `${splitTgl[2]}-${splitTgl[1]}-${splitTgl[0]}`;

        let date = new Date(tanggal_dari);
        ...
        const tanggal_sampai = `${year}-${month}-${day}`;
        
        result = filtersData.filter((word) => word.fTglDr >= tanggal_dari && word.fTglSd <= tanggal_sampai);
...
filterPostData = {
    ...
    bulan: parsePostData.bulan ? `01-${parsePostData.bulan}` : '',
    isLookup: parsePostData.isLookup || true,
    tipeData: typeData
};

urlRequestGrid = `${apiUrl}laporanarusdanapusat/mingguan`
