...
if (typeof pelangganLookupType !== 'undefined' && typeof pelangganLookupData !== 'undefined') {
    typeData = pelangganLookupType;
    filtersData = pelangganLookupData;

    if (parsePostData.jenisorder_id && parsePostData.jenisorder_id != 'undefined') {
        if (parsePostData.jenisorder_id == 2) {
            result = filtersData.filter((word) => word.kodepelanggan.includes('TRANSPORINDO'));
        } else {
            result = filtersData.filter((word) => !word.kodepelanggan.includes('TRANSPORINDO'));
        }
    } else {
...
filterPostData = {
    ...
    fromInput: parsePostData.fromInput || '',
    forLookup: true,
