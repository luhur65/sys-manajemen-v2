...
if (typeof gudangLookupType !== 'undefined' && typeof gudangLookupData !== 'undefined') {
    typeData = gudangLookupType;
    filtersData = gudangLookupData;

    if (parsePostData.pengeluaranstok_id != '' && filtersData.length > 0 && parsePostData.pengeluaranstok_id == filtersData[0].pengeluaranStokGstForLookup) {
        let notIncludeGudangForGst = ['GUDANG KANTOR', 'GUDANG PIHAK III'];
        result = filtersData.filter((word) => !notIncludeGudangForGst.includes(word.gudang));
    } else if (parsePostData.pengeluaranstok_id != '' && filtersData.length > 0 && parsePostData.pengeluaranstok_id == filtersData[0].pengeluaranStokSpkForLookup) {
        let notIncludeGudangForSpk = ['GUDANG KANTOR', 'GUDANG PIHAK III', 'GUDANG SEMENTARA'];
        result = filtersData.filter((word) => !notIncludeGudangForSpk.includes(word.gudang));
    } else {
        result = filtersData
    }
} else {
...
filterPostData = {
    ...
    pengeluaranstok_id: parsePostData.pengeluaranstok_id || '',
    gudangdarike: parsePostData.gudangdarike || '',
    gudangdari_id: parsePostData.gudangdari_id || '',
    gudangke_id: parsePostData.gudangke_id || '',
    isLookup: parsePostData.isLookup || true,
    tipeData: typeData
};

urlRequestGrid = parsePostData.url || `${apiUrl}gudang`
