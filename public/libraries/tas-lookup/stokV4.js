...
if (typeof stokLookupType !== 'undefined' && typeof stokLookupData !== 'undefined') {
    typeData = stokLookupType;
    filtersData = stokLookupData;
} else {
    typeData = 'JSON'
}
var isReload = 'reload'
filterPostData = {
    statusdefault: parsePostData.statusDefault || '',
    aktif: parsePostData.Aktif || '',
    isLookup: true,
    proses: (parsePostData.isReload != 'page') ? isReload : parsePostData.isReload,
    statusreuse: parsePostData.statusreuse || '',
    approveReuse: parsePostData.approveReuse || '',
    penerimaanstok_id: parsePostData.penerimaanstok_id || '',
    pengeluaranstok_id: parsePostData.pengeluaranstok_id || '',
    penerimaanstokheader_nobukti: parsePostData.penerimaanstokheader_nobukti || '',
    from: parsePostData.from || '',
    nobukti: parsePostData.nobukti || '',
    KelompokId: parsePostData.KelompokId || '',
    StokId: parsePostData.StokId || '',
    isLookup: parsePostData.isLookup || true,
    tipeData: typeData
};

urlRequestGrid =  `${apiUrl}stok`;
...
if (typeData == 'LOCAL') {
    ...
    selector.jqGrid({
        ...
        serializeGridData: function(postData) {
            ...
            if (parsePostData.allowedFilterShowAllFirst) {
                input = true
            }
            if (isToolbarSearch) {
