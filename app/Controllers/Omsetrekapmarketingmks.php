...
use App\Models\MomsetrekapmarketingmksModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetrekapmarketingmks extends BaseController
{
    protected $momsetrekapmarketingmksModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->momsetrekapmarketingmksModel = new MomsetrekapmarketingmksModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Rekap Omset Marketing Makassar';
        $data['title'] = 'Laporan Rekap Omset Marketing Makassar';
        
        $data['last_update'] = '';
        $tgllast = $this->momsetrekapmarketingmksModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('omsetrekapmarketingmks/index', $data);
    }
    ...
    public function combomarketing()
    {
        ...
        if ($jenis == 'bln' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingmksModel->getMarketingByBulan($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        } else if ($jenis == 'thn' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingmksModel->getMarketingByTahun($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        }
    ...
    public function grid()
    {
        ...
        $sql = $this->momsetrekapmarketingmksModel->count($where);
        ...
        $data = $this->momsetrekapmarketingmksModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->momsetrekapmarketingmksModel->getGrandTotal($where);
        $tglUpdateData = $this->momsetrekapmarketingmksModel->get_tglupdate();
