...
use App\Models\MOmsetmarketingbtgModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetmarketingbtg extends BaseController
{
    protected $mOmsetmarketingbtgModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->mOmsetmarketingbtgModel = new MOmsetmarketingbtgModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Omset Marketing Bitung';
        $data['title'] = 'Laporan Omset Marketing Bitung';
        
        $data['last_update'] = '';
        $tgllast = $this->mOmsetmarketingbtgModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('Omsetmarketingbtg/index', $data);
    }
    ...
    public function grid()
    {
        ...
        $sql = $this->mOmsetmarketingbtgModel->count($where);
        ...
        $data = $this->mOmsetmarketingbtgModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->mOmsetmarketingbtgModel->getGrandTotal($where);
        $tglUpdateData = $this->mOmsetmarketingbtgModel->get_tglupdate();
