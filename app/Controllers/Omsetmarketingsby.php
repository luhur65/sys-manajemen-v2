...
use App\Models\MOmsetmarketingsbyModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetmarketingsby extends BaseController
{
    protected $mOmsetmarketingsbyModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->mOmsetmarketingsbyModel = new MOmsetmarketingsbyModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Omset Marketing Surabaya';
        $data['title'] = 'Laporan Omset Marketing Surabaya';
        
        $data['last_update'] = '';
        $tgllast = $this->mOmsetmarketingsbyModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('Omsetmarketingsby/index', $data);
    }
    ...
    public function grid()
    {
        ...
        $sql = $this->mOmsetmarketingsbyModel->count($where);
        ...
        $data = $this->mOmsetmarketingsbyModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->mOmsetmarketingsbyModel->getGrandTotal($where);
        $tglUpdateData = $this->mOmsetmarketingsbyModel->get_tglupdate();
