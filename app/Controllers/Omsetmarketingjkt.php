...
use App\Models\MomsetmarketingjktModel;
...
class Omsetmarketingjkt extends BaseController
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        $this->momsetmarketingjktModel = new MomsetmarketingjktModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Omset Marketing Jakarta';
        $data['title'] = 'Laporan Omset Marketing Jakarta';
        
        $data['last_update'] = '';
        $tgllast = $this->momsetmarketingjktModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('omsetmarketingjkt/index', $data);
    }
    ...
    public function grid()
    {
        ...
        $sql = $this->momsetmarketingjktModel->count($where);
        ...
        $data = $this->momsetmarketingjktModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->momsetmarketingjktModel->getGrandTotal($where);
        $tglUpdateData = $this->momsetmarketingjktModel->get_tglupdate();
