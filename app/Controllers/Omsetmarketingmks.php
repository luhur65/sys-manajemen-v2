...
use App\Models\MOmsetmarketingmksModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetmarketingmks extends BaseController
{
    protected $mOmsetmarketingmksModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->mOmsetmarketingmksModel = new MOmsetmarketingmksModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Omset Marketing Makassar';
        $data['title'] = 'Laporan Omset Marketing Makassar';
        
        $data['last_update'] = '';
        $tgllast = $this->mOmsetmarketingmksModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('Omsetmarketingmks/index', $data);
    }
    ...
    public function grid()
    {
        ...
        $sql = $this->mOmsetmarketingmksModel->count($where);
        ...
        $data = $this->mOmsetmarketingmksModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->mOmsetmarketingmksModel->getGrandTotal($where);
        $tglUpdateData = $this->mOmsetmarketingmksModel->get_tglupdate();
