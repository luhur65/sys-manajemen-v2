...
use App\Models\MomsetrekapmarketingbtgModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetrekapmarketingbtg extends BaseController
{
    protected $momsetrekapmarketingbtgModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->momsetrekapmarketingbtgModel = new MomsetrekapmarketingbtgModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Rekap Omset Marketing Bitung';
        $data['title'] = 'Laporan Rekap Omset Marketing Bitung';
        
        $data['last_update'] = '';
        $tgllast = $this->momsetrekapmarketingbtgModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('omsetrekapmarketingbtg/index', $data);
    }
    ...
    public function combomarketing()
    {
        ...
        if ($jenis == 'bln' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingbtgModel->getMarketingByBulan($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        } else if ($jenis == 'thn' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingbtgModel->getMarketingByTahun($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        }
    ...
    public function grid()
    {
        ...
        $sql = $this->momsetrekapmarketingbtgModel->count($where);
        ...
        $data = $this->momsetrekapmarketingbtgModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->momsetrekapmarketingbtgModel->getGrandTotal($where);
        $tglUpdateData = $this->momsetrekapmarketingbtgModel->get_tglupdate();
