...
use App\Models\MomsetrekapmarketingsbyModel;
...
class Omsetrekapmarketingsby extends BaseController
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        $this->momsetrekapmarketingsbyModel = new MomsetrekapmarketingsbyModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Rekap Omset Marketing Surabaya';
        $data['title'] = 'Laporan Rekap Omset Marketing Surabaya';
        
        $data['last_update'] = '';
        $tgllast = $this->momsetrekapmarketingsbyModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('omsetrekapmarketingsby/index', $data);
    }
    ...
    public function combomarketing()
    {
        ...
        if ($jenis == 'bln' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingsbyModel->getMarketingByBulan($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        } else if ($jenis == 'thn' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingsbyModel->getMarketingByTahun($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        }
    ...
    public function grid()
    {
        ...
        $data = $this->momsetrekapmarketingsbyModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->momsetrekapmarketingsbyModel->getGrandTotal($where);
        $tglUpdateData = $this->momsetrekapmarketingsbyModel->get_tglupdate();
