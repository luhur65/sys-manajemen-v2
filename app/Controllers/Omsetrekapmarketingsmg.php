...
use App\Models\MomsetrekapmarketingsmgModel;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;

class Omsetrekapmarketingsmg extends BaseController
{
    protected $momsetrekapmarketingsmgModel;

    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        parent::initController($request, $response, $logger);
        $this->momsetrekapmarketingsmgModel = new MomsetrekapmarketingsmgModel();
    ...
    {
        $data['menuaktif'] = 'Laporan Rekap Omset Marketing Semarang';
        $data['title'] = 'Laporan Rekap Omset Marketing Semarang';
        
        $data['last_update'] = '';
        $tgllast = $this->momsetrekapmarketingsmgModel->get_tglupdate();
        if(!empty($tgllast)) {
            $data['last_update'] = date("d-m-Y h:i:s", strtotime($tgllast[0]->FlastUpdate));
        }
        
        return $this->render('omsetrekapmarketingsmg/index', $data);
    }
    ...
    public function combomarketing()
    {
        ...
        if ($jenis == 'bln' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingsmgModel->getMarketingByBulan($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        } else if ($jenis == 'thn' && !empty($nilai)) {
            $res = $this->momsetrekapmarketingsmgModel->getMarketingByTahun($nilai);
            foreach ($res as $row) {
                $dataArr[] = ['FNMarketing' => $row->FNMarketing];
            }
        }
    ...
    public function grid()
    {
        ...
        $sql = $this->momsetrekapmarketingsmgModel->count($where);
        ...
        $data = $this->momsetrekapmarketingsmgModel->get($where, $sidx, $sord, $limit, $start);
        $grandTotal = $this->momsetrekapmarketingsmgModel->getGrandTotal($where);
        $tglUpdateData = $this->momsetrekapmarketingsmgModel->get_tglupdate();
